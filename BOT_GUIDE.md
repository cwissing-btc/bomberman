...
Run the server, run two copies of this, press **Start** in a player client (or
the moderation UI), and you have a match. Everything below is reference.
...
```
   ...
   a player (code 10) or the moderator starts
...
If the lobby is full or locked, or a match is under way, you get no
`ASSIGNED`. Retry the hello every half second until you get one: new players
are seated in the open lobby and on the results screen after a match.
...
| `10` | `START` | lobby request: start the match (see below) |
| `11` | `PAUSE` | lobby request: freeze the running match for everyone |
| `12` | `RESUME` | lobby request: unfreeze it |
| `13` | `ABORT` | lobby request: end the running match now |
| `14` | `SEAT_BOT` | lobby request: fill a seat with a server bot, or remove it — **4 bytes**, see below |
...
### Lobby requests (codes 10–13)

Any seated player may start, pause, resume or abort a match. This is how the
player client's Start button works; a bot may use it too. The rules:

- Believed only from the address that holds the seat, exactly like a move.
- Executed the moment the packet arrives, not at the tick. The sequence nibble
  still counts, so a duplicated datagram runs once.
- They carry no move: a request in the same tick window does not cancel a
  movement you sent before it.
- `START` in the lobby begins the countdown if `can_start` is set. On the
  results screen (`match over`) it clears the results and counts down to the
  next match in one step, provided enough players are seated.
- `PAUSE` is refused outside a running match.
- `ABORT` ends the match with a normal `MATCH_END`, reason `2` (aborted).
- The server can switch all four off (`lobby.player_control = false`, e.g. for
  a tournament). Bit 2 of the `LOBBY_STATUS` flags says whether they are on.
- There is no reply. Watch the next `LOBBY_STATUS`, which is sent at once
  whenever a request changed anything. Sending a request two or three times
  on consecutive ticks is safe: all of them are idempotent.

### Server bots (code 14)

The server can fill seats with bots of its own (the player client's built-in
bot, running on the server). Any seated player may switch this per seat. It
is the one action packet with a payload:

```text
byte 0:  player_id               your id, as always
byte 1:  (seq << 4) | 14
...
```python
def seat_bot(seat, enabled):
    return bytes([player_id, (seq << 4) | 14, seat, int(enabled)])
```

- Sequenced, authenticated and switched off by `player_control` exactly like
  codes 10–13. A two-byte packet with code 14 has no seat and is refused.
- Accepted in the open lobby and on the results screen, not while locked or
  while a match is counting down or running.
- The setting belongs to the seat. A bot only takes an **empty** seat; a seat
  held by a player keeps the player, and the bot sits down when they leave.
- **A newcomer displaces a bot.** When every seat is taken and at least one is
  held by a server bot, a hello takes that seat.
- Bots only keep players company. When the last player has left, every bot
  leaves too, a match still running is aborted, and the seats go back to the
  server's configured defaults.
- Bit 3 of the `LOBBY_STATUS` flags says whether the server has bots at all;
  the per-seat flags say which seats hold one and which are set to be filled.
...
| `0x01` | `LOBBY_STATUS` | every 30 ticks while waiting, and whenever the lobby changes |
...
|-----------|------|-------|-------------|-----------------|---------|
| **player id** | `u8` | `0`–`3` | server, on your hello | you join → you are kicked, or dropped after 30 s of silence | Yes. A freed seat is handed to the next bot that says hello. Stable for the whole session, across matches. |
...
### 5.3 `LOBBY_STATUS` (0x01) — 11 bytes, plus a tail
...
| 11 | `u8` | flags — bit 0 `paused`, bit 1 `can_start`, bit 2 `player_control`, bit 3 `server_bots` |
| 12 | `u8` | min players needed to start |
| 13 | `u8` | seat count *N* |
| 14 | *N* × seat | per seat: `u8` flags (bit 0 occupied, bit 1 stale, bit 2 bot, bit 3 bot_fill), `u8` name length, name (UTF-8, ≤ 24 bytes) |

Lobby states: `0` open · `1` locked · `2` countdown · `3` running · `4` match over.

The tail from offset 11 on was appended later. A bot that reads only the first
eleven bytes keeps working; a parser should accept a frame that ends at offset
11 (older servers) as complete.

- `can_start` is the server's own answer to "would Start succeed now". It is
  `false` while paused and on the results screen; there, `START` still works
  when enough seats are filled (see §4).
- `stale` means the seat has sent nothing for longer than the server's
  threshold — the client has probably crashed. A server bot is never stale.
- `bot` means the seat is held by a server bot; `bot_fill` that the seat is
  set to be filled by one (§4, code 14). `occupied` without `bot` but with
  `bot_fill` is a player sitting in a bot seat.
- `paused` is orthogonal to the state: a running match can be paused.

When it is sent: every 30 ticks in the lobby and on the results screen, every
10 ticks during the countdown, **immediately whenever the lobby changes** (a
join, a request, a match starting or ending — also during a match), and every
30 ticks of wall time **while paused**, so a paused client can tell a frozen
match from a lost server.
...
Sent when a match finishes and also when it is aborted, by a moderator's
`end` or a player's `ABORT` (reason `2`).
...
| `START` does nothing | Fewer than `min_players` seated, the match is paused, or `player_control` (flags bit 2) is off. The next `LOBBY_STATUS` shows which. |
...
| Actions are ignored | Byte 0 is not your assigned id. Such packets are dropped silently. |
| Your bot lost its seat | It sent nothing for 30 seconds (`lobby.drop_after_secs`) and was dropped. Send `IDLE` a couple of times a second, and say hello again to get a seat back. |
