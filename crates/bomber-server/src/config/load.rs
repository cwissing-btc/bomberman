...
pub struct ServerConfig {
    ...
    /// Seat bots of the server's own when asked to.
...
pub fn load(path: impl AsRef<Path>) -> Result<ServerConfig> {
    ...
    let max_players = file.lobby.max_players.max(1);
    let seats = max_players.min(bomber_protocol::MAX_PLAYERS as u8);
    let mut bot_seats = 0u8;
    for &seat in &file.lobby.bot_seats {
        if seat >= seats {
            anyhow::bail!("lobby.bot_seats: there is no seat {seat} (seats are 0..{seats})");
        }
        bot_seats |= 1 << seat;
    }
    if bot_seats != 0 && !file.lobby.server_bots {
        anyhow::bail!("lobby.bot_seats needs lobby.server_bots = true");
    }
    ...
    Ok(ServerConfig {
        ...
        server_bots: file.lobby.server_bots,
        session: SessionConfig {
            ...
            drop_after_ticks: ticks(file.lobby.drop_after_secs),
            player_control: file.lobby.player_control,
            bot_seats,
        },
    })
}
