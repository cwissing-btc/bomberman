...
pub struct SlotDto {
    ...
    pub name: String,
    pub connected: bool,
    /// Held by one of the server's own bots. Such a seat has no `addr`.
    pub bot: bool,
    /// Filled by a server bot whenever no player holds it.
    pub bot_fill: bool,
...
pub struct LobbyDto {
    ...
    pub can_start: bool,
    /// Whether `seat_bot` is available on this server.
...
pub fn lobby_dto(snapshot: &SessionSnapshot, registry: &Registry) -> LobbyDto {
    ...
    LobbyDto {
        ...
        server_bots: snapshot.server_bots,
