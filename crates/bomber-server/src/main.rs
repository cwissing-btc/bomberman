...
async fn main() -> Result<()> {
    ...
    tracing::info!(
        ...
        server_bots = config.server_bots,
    ...
    let mut session = ArenaSession::new(config.session, entropy);
    if config.server_bots {
        session = session.with_bots(Box::new(bots::BuiltinBots));
    }
