...
struct Outbox {
    ...
    lobby_changed: bool,
    released: Vec<bomber_domain::shared::PlayerId>,
}
...
fn build_outbox(shared: &Shared) -> Outbox {
...

    {
        ...
        outbox.released = report.released;
    ...
    // Players the session dropped for silence: forget their addresses, as for
    // a kick, so a late datagram cannot drive whoever gets the seat next.
    if !outbox.released.is_empty() {
        let mut registry = shared.registry.lock().unwrap();
        for player in &outbox.released {
            let addr = registry.addr_of(*player);
            registry.release(*player);
            tracing::info!(%player, ?addr, "dropped a silent player");
        }
    }
