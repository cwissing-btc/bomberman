...
impl InputSlots {
    ...
    pub fn submit(
        ...
        let slot = Self::accept(&mut self.slots, player, seq)?;
        slot.pending = Some(action.intent());
        Ok(())
    }

    /// Record a packet that is not a move -- a lobby request -- so the
    /// sequence bookkeeping stays exact, without replacing this tick's intent.
    ///
    /// Returns `Err(Stale)` for a duplicate, which is what keeps one request
    /// that UDP happened to deliver twice from being executed twice.
    pub fn note(&mut self, player: PlayerId, seq: u8) -> Result<(), Rejection> {
        Self::accept(&mut self.slots, player, seq).map(|_| ())
    }

    fn accept(slots: &mut [Slot], player: PlayerId, seq: u8) -> Result<&mut Slot, Rejection> {
        let slot = slots
