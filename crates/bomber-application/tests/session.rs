...
fn presence_notices_a_bot_that_has_gone_quiet() {
    ...
    // Seat 1 never sent an action; its silence counts from the hello.
    assert!(snapshot.seats[1].stale, "seat 1 has been quiet since joining");
    assert_eq!(snapshot.seats[1].presence.packets_total, 0);
}
