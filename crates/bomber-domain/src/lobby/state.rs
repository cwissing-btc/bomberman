...
impl LobbyState {
    ...
    /// Whether a hello from a new address gets a seat.
    ///
    /// Also on the results screen: a player may start the next match straight
    /// from there (reset and countdown in one step), so without this a latecomer
    /// would find no moment to join for as long as the others keep playing.
    /// Joining there changes nothing about the finished match.
    pub const fn admits_new_players(self) -> bool {
        matches!(self, LobbyState::Open | LobbyState::MatchOver)
    }
}
