...
pub struct Seat {
    pub id: PlayerId,
    pub occupied: bool,
    /// Held by one of the server's own bots rather than a connected player.
    /// Implies `occupied`.
    pub bot: bool,
    /// Fill this seat with a server bot whenever no player holds it. A setting
    /// of the seat, not of its occupant: it survives the occupant leaving.
    pub bot_fill: bool,
...
impl Seat {
    ...
    /// The name a server bot plays under. Numbered from 1 like the seats a
    /// person sees in the lobby.
    pub fn bot_name(id: PlayerId) -> String {
        format!("Server-Bot {}", id.raw() + 1)
    }

    /// Whether a player arriving now may take this seat: it is free, or only
    /// a server bot is keeping it warm.
    pub fn is_open_to_players(&self) -> bool {
        !self.occupied || self.bot
    }
...
mod tests {
    ...
    #[test]
    fn leaving_a_seat_keeps_its_bot_setting() {
        let mut seat = Seat::empty(PlayerId::new(1));
        seat.bot_fill = true;
        seat.occupied = true;
        seat.bot = true;
        seat.vacate();
        assert!(!seat.bot);
        assert!(seat.bot_fill, "the setting belongs to the seat");
    }
}
